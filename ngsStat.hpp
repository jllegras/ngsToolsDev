...
// print help
void info() {
  fprintf(stdout, "-npop: how many pops (1 or 2); -postfiles: .sfs files with posterior probabilities of sample allele frequencies for each population (with or without running sfstools); -outfile: name of the output file; -nind: number of individuals for each population; -nsites: total number of sites; in case you want to analyze a subset of sites this is the upper limit; -verbose: level of verbosity, if 0 suppress all messages; -block_size: to be memory efficient, set this number as the number of sites you want to analyze at each chunk; -firstbase: in case you want to analyze a subset of your sites this is the lower limit; -isfold: boolean, is your data folded or not? -islog: boolean, are postfiles in -log (from -realSFS 1 only, required if 2D-SFS is given)? If you use sfstools then -islog 1; -iswin: if 1 then print the value computed for each non-overlapping window defined by block_size.");
}
